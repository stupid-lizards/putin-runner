...
function MyModal(modal_type, valid_attempt, that) {
  that.togglePopup(true);
  if (modal_type == "myModalForm") {
    var link = "https://nwp0c66u442.typeform.com/to/dH0bXagM";
    var modal = document.getElementById(modal_type);
    if (valid_attempt == 1) {
      var continueBtn = document.getElementById("continue_btn");
      continueBtn.innerHTML += `<a href=${link} target="_blank" class="button">Continue</a>`;
    }
  ...
  class Runner {
    ...
    onKeyDown(e) {
      ...
      if (this.popup) {
        ...
        if (IS_MOBILE && this.playing && !this.popup) {
          e.preventDefault();
        }

        if (e.target != this.detailsButton && !this.popup) {
          if (
            !this.crashed &&
      ...
      if (this.distanceRan > this.threshold) {
        if (this.valid_attempt >= 1) {
          if (this.distanceRan > this.highestScore) {
            MyModal("myModalForm", this.valid_attempt, this);
            this.valid_attempt += 1;
          } else {
            MyModal("myModal", 0, this);
          }
        } else {
          this.valid_attempt += 1;
          MyModal("myModal", 0, this);
